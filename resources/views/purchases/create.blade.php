			...
			<div class="panel-body">
				...
				<div class="row" id="ledgerRow" style="display:none;">
					...
					<div class="form-group" style="margin-left: 1%; margin-right: 1%;"> 
						...
						<div class="col-sm-5" style="height: 40px;"> 
						{!! Form::select('suppliers_id', $Account, null, ['id' => 'suppliers_id', 'onchange' =>'GetSupplierID($(this).val());', 'class'=>'form-control livesearch']) !!}
...
function PriceKeyUp(price){
    ...
    if (e) {
		...
		tableHtml += `<td style="padding-top:20px;"><input id="test" value="${ProductCode}" type="text" class="form-control" style="margin-left: 7%; width: 50%;" disabled></td>`;
		//tableHtml += '<td>'+ ProductName +'</td>';
		tableHtml += `<td style="padding-top:20px;"><input id="test" value="${ProductName}" type="text" class="form-control" style="margin-left:-29%; width: 155%;" disabled></td>`;
		...
		tableHtml += `<td style="padding-top:20px;"><input id="test" value="${Quantity}" type="text" class="form-control" style="margin-left:4%; width: 52%;" disabled></td>`;
		...
		tableHtml += `<td style="padding-top:20px; display:none;"><input id="test" value="${warehouse_id}" type="text" class="form-control" style="margin-left:-32%; width: 103%;" disabled></td>`;
		//tableHtml += '<td>'+ Total +'</td>';
		tableHtml += `<td style="padding-top:20px;"><input id="test" value="${Total}" type="text" class="form-control" style="margin-left:-22%; width: 103%;" disabled></td>`;
		tableHtml += '<td><button class="btn btn-red" type="button" style="margin-left: -45%;"> <i onclick="javascript:myDeleteFunction($(this).closest(\'tr\'));" class="icon-trash" title="Delete Row"></i></button></td>';
		tableHtml += '</tr></br>';
...
$("#btnSave").click(function()
{
	...
	$.each($("#myData tr"), function(index, row){
		...
		product.expiry = $(columns[7]).find("input").val();
		product.warehouse_id = $(columns[8]).find("select").val();
		product.total_cost = $(columns[9]).find("input").val();
